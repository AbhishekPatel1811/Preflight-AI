...
export function formatPreflightReportMarkdown(input: PreflightInput, result: PreflightResult) {
  ...
  const lines = [
    ...
    "## Landing Lens",
    "",
    `- Landing score: ${report.landingLens?.score === null || report.landingLens?.score === undefined ? "Not scored" : `${report.landingLens.score}/100`}`,
    `- Evidence status: ${report.landingLens?.status ?? "not captured"}`,
    ...(report.landingLens?.criteria.map(
      (criterion) => `- ${criterion.label}: ${criterion.score === null ? "Not scored" : `${criterion.score}/100`}\n  - ${criterion.evidence}`
    ) ?? []),
    "",
    "## Launch context",
    "",
    ...
    `- Why this CTA: ${result.landingRecommendations.ctaRationale}`,
    "",
    "### Proof plan",
    "",
    ...result.landingRecommendations.proofRecommendations.map((recommendation) => `- ${recommendation}`),
    "",
    "## Follow-up questions",
    "",
